...
const Button = ({ label = "Button", onClick = () => {}, variant = 'primary', className, disabled = false, leftIcon, rightIcon}) => {
  ...
  const variantStyle = variant === 'primary' ? 'flex justify-center items-center gap-2 bg-blue-500 text-white' : 'flex justify-center items-center gap-2 bg-gray-500 text-white';
  const disabledStyle = disabled ? 'flex justify-center items-center gap-2 opacity-50 cursor-not-allowed' : 'flex justify-center items-center gap-2 hover:bg-opacity-90';
